...
class YangParseModelsAction(Action):
    ...
    def run(self, device_name, yang_path=None, workers=10, store_in_datastore=True):
        ...
        try:
            ...
            self.logger.info(f"Extracted {total_paths} paths from {len(path_catalog)} modules in {parse_time:.2f}s")
            ...
            self.logger.error("Hint: Run yang_download_models first to download YANG files")
            return (False, {
                'success': False,
                'error': str(e),
    ...
    def _log_parse_statistics(self, path_catalog):
        ...
        config_count = sum(
            1 for mod_data in path_catalog.values()
        ...
        for i, (module_name, data) in enumerate(modules_by_path_count[:10], 1):
            self.logger.info(f"  {i:2d}. {module_name:40s} - {data['path_count']:4d} paths")

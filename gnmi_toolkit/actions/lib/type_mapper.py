    ...
    def map_yang_to_st2_parameter(self, path_metadata):
        ...
        if 'length' in path_metadata:
            ...
            if 'min' in length_info or 'max' in length_info:
                ...
                param_spec['description'] = param_spec.get('description', '') + length_desc
        ...
        if 'union_types' in path_metadata:
            ...
            param_spec['description'] = param_spec.get('description', '') + f" (union: {union_types})"
        
        # Leafref - add path reference in description
        if 'leafref_path' in path_metadata:
            param_spec['description'] = param_spec.get('description', '') + f" (ref: {path_metadata['leafref_path']})"
        
        # Identity reference
        if 'identity_base' in path_metadata:
            param_spec['description'] = param_spec.get('description', '') + f" (identity: {path_metadata['identity_base']})"
